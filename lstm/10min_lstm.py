...
from model import TimeSeriesDataset, LSTMModel
...
for location, location_data in final_averaged_data.groupby('LocationCode'):
    ...
    # 按日期進行分組
    grouped_data = location_data.groupby(location_data.index.date)

    # 將日期轉為列表以便按順序訪問
    dates = list(grouped_data.groups.keys())
    for i in range(1, len(dates)):  # 從第二天開始，確保有前一天的數據
        current_date = dates[i]
        previous_date = dates[i - 1]

        # 獲取前一天和當天的數據
        previous_day_data = grouped_data.get_group(previous_date)
        current_day_data = grouped_data.get_group(current_date)

        # 確保兩天的數據都有完整的 60 筆
        if len(previous_day_data) >= 24 and len(current_day_data) == 60:
            # 合併前一天和當天的數據
            combined_data = pd.concat([previous_day_data[-24:], current_day_data])

            # 提取當天的特徵 (前 12 筆數據)
            features = combined_data.iloc[:36][[
            ...
            # 提取當天未來 48 步的目標 (Power)
            target = combined_data.iloc[36:]['Power(mW)'].values
...
X_train_final, X_val, y_train_final, y_val = train_test_split(X_train_val, y_train_val, test_size=0.2, random_state=42)
...
model = LSTMModel(input_size=X_train.shape[2], hidden_size=256, output_size=48)
model.to(device)
...
for i in range(1, 18):
    ...
    for date in time_temp['day']:
        ...
        # 確保有 30 步數據
        # if len(input_data) < 72:
        #     raise ValueError("輸入數據不足 12 步，請檢查時間範圍是否正確。")

        # 取前 30 步的特徵作為模型輸入
        input_features = input_data.iloc[-36:][[
                'WindSpeed(m/s)', 'Pressure(hpa)', 'Temperature(°C)',
                'Humidity(%)', 'Sunlight(Lux)', 'Power(mW)',
                'LocationCode_1.0', 'LocationCode_2.0', 'LocationCode_3.0', 'LocationCode_4.0',
                'LocationCode_5.0', 'LocationCode_6.0', 'LocationCode_7.0', 'LocationCode_8.0',
...
# 保存為 CSV
final_df.to_csv("123.csv", index=False)